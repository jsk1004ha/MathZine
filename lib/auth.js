...
function normalizeStatus(status) {
  return sanitizeText(status, { maxLength: 20 }).toLowerCase() === "banned" ? "banned" : "active";
}
...
export function assertUserNotBanned(user) {
  if (isUserBanned(user)) {
    const error = new Error(user.banReason || "이 계정은 제한되어 있습니다. 관리자에게 문의해 주세요.");
...
export function canEditArticle(user, article) {
  return Boolean(
    user &&
      article &&
      (user.role === "admin" || (user.role === "reporter" && article.authorId === user.id))
  );
}

export function canCreateHallProblemForArticle(user, article) {
  return Boolean(user && article && (user.role === "admin" || (user.role === "reporter" && article.authorId === user.id)));
}

export function canGradeHallProblem(user, problem) {
  return Boolean(user && problem && (user.role === "admin" || user.role === "teacher" || problem.authorId === user.id));
}
...
export function canPreviewArticle(user, article) {
  return Boolean(user && article && (canManageEditorial(user) || user.id === article.authorId));
}
...
export async function loginUser(loginId, password, options = {}) {
  ...
  assertUserNotBanned(existingUser);
...
export async function updateUserBanStatus(targetUserId, nextStatus, reason, actorUserId) {
  const status = normalizeStatus(nextStatus);
  const [users, sessions] = await Promise.all([readCollection("users"), readCollection("sessions")]);
  const index = users.findIndex((user) => user.id === targetUserId);
  ...
  if (users[index].role === "admin" && status === "banned") {
    throw new Error("admin 계정은 밴할 수 없습니다.");
  }

  users[index] = {
    ...users[index],
    status,
    banReason: status === "banned" ? sanitizeText(reason, { maxLength: 200, multiline: true }) || "운영 정책 위반" : "",
    bannedAt: status === "banned" ? nowIso() : null,
    bannedByUserId: status === "banned" ? actorUserId : null
  };
  ...
  if (status === "banned") {
    await writeCollection(
      "sessions",
      sessions.filter((session) => session.userId !== targetUserId)
    );
  }
...
export async function deleteUserAccount(targetUserId, actorUserId) {
  if (targetUserId === actorUserId) {
    throw new Error("현재 로그인한 본인 계정은 삭제할 수 없습니다.");
  }

  const [users, sessions, articles, boardPosts, comments, submissions] = await Promise.all([
  ...
  if (targetUser.role === "admin" && users.filter((user) => user.role === "admin").length === 1) {
    throw new Error("마지막 admin 계정은 삭제할 수 없습니다.");
  }
      ...
      articles.map((article) =>
        article.authorId === targetUserId
          ? {
              ...article,
              authorId: null,
              authorNickname: anonymizedName,
              likeUserIds: (article.likeUserIds ?? []).filter((userId) => userId !== targetUserId)
            }
          : {
              ...article,
              likeUserIds: (article.likeUserIds ?? []).filter((userId) => userId !== targetUserId)
...
export async function changeLocalPassword(userId, currentPassword, newPassword) {
  ...
  if (user.authProvider !== "local") {
    throw new Error("리로스쿨 계정은 여기서 비밀번호를 바꿀 수 없습니다.");
  }
  ...
  const safePassword = sanitizeText(newPassword, { maxLength: 80 });
...
export async function adminResetLocalPassword(targetUserId, newPassword) {
  const users = await readCollection("users");
  const index = users.findIndex((user) => user.id === targetUserId);
  ...
  if (users[index].authProvider !== "local") {
    throw new Error("local 계정만 재설정할 수 있습니다.");
  }
