...
async function inlineUploadedMediaSourcesInHtml(html) {
  const source = String(html ?? "");
  const pattern = /\bsrc\s*=\s*(["'])(\/api\/media\/[^"']+)\1/gi;
  let lastIndex = 0;
  let nextHtml = "";

  for (const match of source.matchAll(pattern)) {
    const src = match[2];
    const renderableSrc = await mediaSourceToRenderableSrc(src);
    nextHtml += source.slice(lastIndex, match.index);
    nextHtml += `src=${match[1]}${escapeAttr(renderableSrc || src)}${match[1]}`;
    lastIndex = match.index + match[0].length;
  }

  return `${nextHtml}${source.slice(lastIndex)}`;
}
...
async function renderHtmlDocumentFrame(document) {
  const html = await inlineUploadedMediaSourcesInHtml(document.html);

  return `
    <iframe
      class="print-block print-html-frame"
      referrerpolicy="no-referrer"
      sandbox=""
      srcdoc="${escapeAttr(html)}"
      style="height:${Number(document.htmlHeight) || 720}px"
...
async function renderArticleSection(article) {
  ...
  if (normalized.mode === "html") {
    blocks.push(await renderHtmlDocumentFrame(normalized));
  } else {
