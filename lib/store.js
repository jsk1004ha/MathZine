...
const legacyFilePaths = {
  ...
  auditLogs: path.join(storageDir, "audit-logs.json"),
  rateLimits: path.join(storageDir, "rate-limits.json")
};
...
const tableConfigs = {
  ...
  rateLimits: { table: "rate_limits", key: "id" }
};
...
async function migrateStorage() {
  ...
  const nextUsers = users.map((user, index) => {
    ...
    if (!nextUser.banReason) {
      nextUser.banReason = "";
      changedUsers = true;
    }

    if (typeof nextUser.bannedAt === "undefined") {
      nextUser.bannedAt = null;
      changedUsers = true;
    }

    if (typeof nextUser.bannedByUserId === "undefined") {
      nextUser.bannedByUserId = null;
      changedUsers = true;
    }
  ...
  const nextProblems = problems.map((problem) => {
    ...
    if (!nextProblem.type) {
      nextProblem.type = "essay";
      changedProblems = true;
    }

    if (!Array.isArray(nextProblem.choices)) {
      nextProblem.choices = [];
      changedProblems = true;
    }

    if (typeof nextProblem.correctAnswer === "undefined") {
      nextProblem.correctAnswer = "";
      changedProblems = true;
    }

    if (typeof nextProblem.scoringGuide === "undefined") {
      nextProblem.scoringGuide = "";
      changedProblems = true;
    }

    if (!nextProblem.points) {
      nextProblem.points = 10;
      changedProblems = true;
    }

    if (typeof nextProblem.authorId === "undefined") {
      const article = nextArticles.find((entry) => entry.slug === nextProblem.articleSlug);
      nextProblem.authorId = article?.authorId ?? null;
      nextProblem.authorNickname = article?.authorNickname ?? nextProblem.authorNickname ?? "";
      changedProblems = true;
    }
  ...
  for (const article of nextArticles) {
    ...
    if (typeof nextIssue.coverImageSrc === "undefined") {
      nextIssue.coverImageSrc = null;
      changedIssues = true;
    }
  ...
  const nextSubmissions = submissions.map((submission) => {
    ...
    if (!nextSubmission.fileKind) {
      nextSubmission.fileKind = nextSubmission.originalFileName?.toLowerCase().match(/\.(png|jpe?g|webp)$/)
        ? "image"
        : "pdf";
      changedSubmissions = true;
    }

    if (!nextSubmission.mimeType) {
      nextSubmission.mimeType =
        nextSubmission.fileKind === "image"
          ? nextSubmission.originalFileName?.toLowerCase().endsWith(".png")
            ? "image/png"
            : nextSubmission.originalFileName?.toLowerCase().endsWith(".webp")
              ? "image/webp"
              : "image/jpeg"
          : "application/pdf";
      changedSubmissions = true;
    ...
    if (!nextSubmission.problemType) {
      nextSubmission.problemType = "essay";
      changedSubmissions = true;
    }

    if (typeof nextSubmission.answerText === "undefined") {
      nextSubmission.answerText = "";
      changedSubmissions = true;
    }

    if (typeof nextSubmission.autoGraded === "undefined") {
      nextSubmission.autoGraded = false;
      changedSubmissions = true;
    }

    if (typeof nextSubmission.isCorrect === "undefined") {
      nextSubmission.isCorrect = null;
      changedSubmissions = true;
    }

    if (typeof nextSubmission.gradedByUserId === "undefined") {
      nextSubmission.gradedByUserId = null;
      changedSubmissions = true;
    }

    if (typeof nextSubmission.gradedAt === "undefined") {
      nextSubmission.gradedAt = null;
      changedSubmissions = true;
    }
  ...
  if (auditLogs.length > 1000) {
    await writeCollection(
      "auditLogs",
      auditLogs.slice(-1000)
    );
  }

  const rateLimits = await readCollection("rateLimits");
  const now = Date.now();
  const activeRateLimits = rateLimits.filter((entry) => new Date(entry.resetAt).getTime() > now);

  if (activeRateLimits.length !== rateLimits.length) {
    await writeCollection("rateLimits", activeRateLimits);
  }
}
