...
export const HTML_FRAME_BOTTOM_GUARD = 16;
...
export function calculateHtmlFrameContentHeight(contentBottom, viewportHeight, bottomGuard = HTML_FRAME_BOTTOM_GUARD) {
  const content = Number(contentBottom);
  const viewport = Number(viewportHeight);
  const guard = Number(bottomGuard);
  const safeContent = Number.isFinite(content) && content > 0 ? content : 0;
  const safeViewport = Number.isFinite(viewport) && viewport > 0 ? viewport : 0;
  const safeGuard = Number.isFinite(guard) && guard > 0 ? guard : 0;

  if (safeContent > safeViewport + 1) {
    return Math.ceil(safeContent + safeGuard);
  }

  return Math.ceil(Math.max(safeContent, safeViewport));
}
...
<script data-mathzine-frame-resize>
  (() => {
    ...
    const calculateHeight = ${calculateHtmlFrameContentHeight.toString()};
    const bottomGuard = ${HTML_FRAME_BOTTOM_GUARD};
    ...
    const getVisualContentBottom = (body) => {
      if (!body) return 0;

      let maxBottom = body.getBoundingClientRect().bottom || 0;
      ...
      while (textNode) {
        if (textNode.nodeValue?.trim()) {
          const parentStyle = textNode.parentElement ? getComputedStyle(textNode.parentElement) : null;

          if (!parentStyle || (parentStyle.display !== "none" && parentStyle.visibility !== "hidden" && Number(parentStyle.opacity) !== 0)) {
    ...
    const measure = () => {
      ...
      const contentBottom = Math.max(
        ...
        bodyRect?.height || 0,
        bodyRect?.bottom || 0,
        getVisualContentBottom(body)
      );
      const height = calculateHeight(contentBottom, viewportHeight, bottomGuard);
