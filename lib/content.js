...
import {
  assertUserNotBanned,
  canCreateHallProblemForArticle,
  canCreateNotice,
  canEditArticle,
  canGradeHallProblem,
  canPreviewArticle,
...
import { sanitizeText, slugify, splitParagraphs, withErrorCode } from "@/lib/security";
...
function normalizeHallProblemType(type) {
  const normalized = sanitizeText(type, { maxLength: 30 });

  if (normalized === "multiple_choice" || normalized === "short_answer" || normalized === "essay") {
    return normalized;
  }
...
function normalizeAnswerText(value) {
  return sanitizeText(value, { maxLength: 500, multiline: true })
    .trim()
    .replace(/\s+/g, " ")
    .toLowerCase();
}

function sanitizeProblemChoices(value) {
  ...
  return [...new Set(source.map((choice) => sanitizeText(choice, { maxLength: 160 })).filter(Boolean))].slice(0, 8);
}

function sanitizeProblemForViewer(problem, viewer) {
  if (!problem) {
    return problem;
  }

  const canGrade = canGradeHallProblem(viewer, problem);
  const safeProblem = {
    ...problem,
    type: normalizeHallProblemType(problem.type),
    choices: Array.isArray(problem.choices) ? problem.choices : [],
    points: Number(problem.points ?? 10) || 10,
    canGrade
  };

  if (!canGrade) {
    delete safeProblem.correctAnswer;
    delete safeProblem.scoringGuide;
    delete safeProblem.submissions;
  }

  return safeProblem;
}

function resolveIssueForSubmission(issues, payload) {
  const issueSlug = normalizeIssueSlug(payload.issueSlug || payload.issue);
  const issue = issues.find((entry) => matchesIssueSlug(entry, issueSlug));

  if (!issue) {
    throw new Error("먼저 어드민 또는 교사가 만든 호수를 선택해 주세요.");
  }

  if (issue.status === "published") {
    throw new Error("이미 공개된 호수에는 새 기사를 제출할 수 없습니다. 편집부가 새 호수를 만들어야 합니다.");
  }

  return issue;
}

function resolveIssueForUpdate(issues, payload) {
  const issueSlug = normalizeIssueSlug(payload.issueSlug || payload.issue);
  const issue = issues.find((entry) => matchesIssueSlug(entry, issueSlug));
...
function sanitizeArticleInput(payload) {
  ...
  const issueInput = sanitizeText(payload.issue, { maxLength: 80 });
  ...
  const hasDocumentContent = document.mode === "html" ? Boolean(document.html) : document.blocks.length >= 1;

  if (!title || !deck || !section || !tag || !readTime || !hasDocumentContent || paragraphs.length < 1) {
    throw new Error("기사 제목, 부제, 섹션, 태그, 호수, 읽기 시간, 본문 블록이 필요합니다.");
  }

  return { title, deck, section, tag, issueInput, readTime, document, paragraphs };
}
...
export async function getArticleBySlug(slug, options = {}) {
  ...
  ).map((problem) => sanitizeProblemForViewer(problem, viewer));
...
export async function deleteBoardPost(postId) {
  const safePostId = sanitizeText(postId, { maxLength: 120 });
  const posts = await readCollection("boardPosts");
  const targetPost = posts.find((post) => post.id === safePostId);
  ...
  await writeCollection(
    "boardPosts",
    posts.filter((post) => post.id !== safePostId)
  );
...
export async function createArticle(payload, user) {
  ...
  const { title, deck, section, tag, issueInput, readTime, document, paragraphs } = sanitizeArticleInput(payload);
  ...
  const selectedIssue = resolveIssueForSubmission(issues, { issueSlug: payload.issueSlug, issue: issueInput });
  const issue = selectedIssue.issue;
  const issueSlug = selectedIssue.issueSlug;
...
export async function updateArticle(slug, payload, user) {
  assertUserNotBanned(user);
  const safeSlug = sanitizeText(slug, { maxLength: 120 });
  const [articles, issues] = await Promise.all([readCollection("articles"), readCollection("issues")]);
  const articleIndex = articles.findIndex((article) => article.slug === safeSlug);

  if (articleIndex === -1) {
    throw withErrorCode(new Error("기사를 찾을 수 없습니다."), "ARTICLE_NOT_FOUND", 404);
  }

  const currentArticle = articles[articleIndex];

  if (!canEditArticle(user, currentArticle)) {
    throw withErrorCode(new Error("기사 수정 권한이 없습니다."), "FORBIDDEN", 403);
  }

  const { title, deck, section, tag, issueInput, readTime, document, paragraphs } = sanitizeArticleInput(payload);
  const selectedIssue = resolveIssueForUpdate(issues, { issueSlug: payload.issueSlug, issue: issueInput });

  if (currentArticle.status === "published" && !articleBelongsToIssue(currentArticle, selectedIssue)) {
    throw new Error("공개된 기사의 호수는 변경할 수 없습니다.");
  }
  ...
  const issueIndex = issues.findIndex((issue) => issue.issueSlug === selectedIssue.issueSlug);
...
export async function createIssue(payload) {
  const issue = sanitizeText(payload.issue, { maxLength: 80 });
  const issueSlug = normalizeIssueSlug(payload.issueSlug || issue);

  if (!issue || !issueSlug) {
    throw new Error("호수 이름을 입력해 주세요.");
  }

  const issues = await readCollection("issues");

  if (issues.some((entry) => matchesIssueSlug(entry, issueSlug))) {
    throw new Error("이미 같은 호수가 있습니다.");
  }

  const nextIssue = createIssueRecord(issue, issueSlug);
  issues.push(nextIssue);
  await writeCollection("issues", issues);
  return nextIssue;
}

export async function deleteIssue(issueSlug) {
  ...
  if (articles.some((article) => articleBelongsToIssue(article, issue))) {
    throw new Error("연결된 기사가 있어 삭제할 수 없습니다. 먼저 기사를 삭제하거나 다른 호수로 옮겨 주세요.");
  }

  await writeCollection(
    "issues",
    issues.filter((_, index) => index !== issueIndex)
  );
  return issue;
}
  ...
  const mapped = issues
    ...
    .map((issue) => {
      ...
      return {
        ...
        coverImageSrc: issue.coverImageSrc || null,
        status: issue.status,
...
export async function listWritableIssues() {
  const issues = await readCollection("issues");
  return sortByDate(
...
export async function unpublishIssue(issueSlug) {
  const [issues, articles] = await Promise.all([readCollection("issues"), readCollection("articles")]);
  const issueIndex = issues.findIndex((issue) => matchesIssueSlug(issue, issueSlug));
  ...
  const nextArticles = articles.map((article) =>
    articleBelongsToIssue(article, targetIssue)
      ? {
          ...article,
          status: "submitted",
...
export async function updateIssueCover(issueSlug, coverImageSrc) {
  const issues = await readCollection("issues");
  const issueIndex = issues.findIndex((issue) => matchesIssueSlug(issue, issueSlug));
...
export async function deleteArticle(slug) {
  const [articles, comments, problems] = await Promise.all([
    readCollection("articles"),
    readCollection("comments"),
    readCollection("hallProblems")
  ]);
  ...
  const detachedProblemIds = new Set(problems.filter((problem) => problem.articleSlug === slug).map((problem) => problem.id));
  const nextArticles = articles.filter((entry) => entry.slug !== slug);
  const nextComments = comments.filter((comment) => comment.articleSlug !== slug);
  const nextProblems = problems.map((problem) =>
    detachedProblemIds.has(problem.id)
      ? {
          ...problem,
          articleSlug: null
        }
  ...
  await Promise.all([
    writeCollection("articles", nextArticles),
    writeCollection("comments", nextComments),
    writeCollection("hallProblems", nextProblems)
  ]);

  return {
    article,
    removedCommentCount: comments.length - nextComments.length,
    detachedProblemCount: detachedProblemIds.size
  };
}
...
export async function getIssueBundle(issueSlug, options = {}) {
  ...
  const issueArticles = articles
    .filter((article) => articleBelongsToIssue(article, issue))
    .filter((article) => !includeUnpublished || canPreviewArticle(viewer, article));
  ...
  return sortByDate(
    problems
      .map((problem) => {
        ...
        return {
          ...sanitizeProblemForViewer(problem, viewer),
          article
        };
      })
      .filter(Boolean),
...
export async function createHallProblem(payload, user) {
  assertUserNotBanned(user);
  ...
  const type = normalizeHallProblemType(payload.type);
  const choices = sanitizeProblemChoices(payload.choices);
  const correctAnswer = sanitizeText(payload.correctAnswer, { maxLength: 500, multiline: true });
  const scoringGuide = sanitizeText(payload.scoringGuide, { maxLength: 1000, multiline: true });
  const points = Math.max(1, Math.min(100, Number(payload.points ?? 10) || 10));
  ...
  if (!canCreateHallProblemForArticle(user, article)) {
    throw new Error("기자는 본인이 작성한 기사에만 문제를 만들 수 있습니다.");
  }

  if (type === "multiple_choice" && choices.length < 2) {
    throw new Error("객관식 문제는 선택지를 2개 이상 입력해 주세요.");
  }

  if ((type === "multiple_choice" || type === "short_answer") && !correctAnswer) {
    throw new Error("객관식/주관식 자동 채점을 위해 정답을 입력해 주세요.");
  }

  if (type === "multiple_choice" && !choices.some((choice) => normalizeAnswerText(choice) === normalizeAnswerText(correctAnswer))) {
    throw new Error("객관식 정답은 선택지 중 하나와 정확히 같아야 합니다.");
  }
  ...
  const problem = {
    ...
    type,
    choices,
    correctAnswer: type === "essay" ? "" : correctAnswer,
    scoringGuide,
    points,
    authorId: user.id,
    authorNickname: user.nickname,
...
export async function listHallSubmissions(options = {}) {
  const { problemIds = null } = options;
  const [submissions, problems] = await Promise.all([
    readCollection("hallSubmissions"),
    readCollection("hallProblems")
  ]);
  const problemIdSet = Array.isArray(problemIds) ? new Set(problemIds) : null;

  return sortByDate(
    submissions.filter((submission) => !problemIdSet || problemIdSet.has(submission.problemId)),
...
export async function createHallSubmission(payload, user) {
  ...
  const fileKind = sanitizeText(payload.fileKind, { maxLength: 20 }) || (storedFileName ? "pdf" : "text");
  const mimeType = sanitizeText(payload.mimeType, { maxLength: 80 }) || (storedFileName ? "application/pdf" : "text/plain");
  const fileSize = Number(payload.fileSize ?? 0);
  const answerText = sanitizeText(payload.answerText, { maxLength: 5000, multiline: true });
  const selectedChoice = sanitizeText(payload.selectedChoice, { maxLength: 500 });

  if (!problemId) {
    throw new Error("연결된 문제 정보가 필요합니다.");
  }
  ...
  const type = normalizeHallProblemType(problem.type);
  const submittedAnswer = type === "multiple_choice" ? selectedChoice : answerText;

  if (type === "multiple_choice" && !selectedChoice) {
    throw new Error("객관식 답을 선택해 주세요.");
  }

  if (type === "multiple_choice" && !(problem.choices ?? []).some((choice) => choice === selectedChoice)) {
    throw new Error("유효한 객관식 선택지를 골라 주세요.");
  }

  if (type === "short_answer" && !answerText) {
    throw new Error("주관식 답을 입력해 주세요.");
  }

  if (type === "essay" && !answerText && !storedFileName) {
    throw new Error("서술형 풀이는 글, 파일, 온라인 노트 중 하나로 제출해 주세요.");
  }

  const autoGraded = type === "multiple_choice" || type === "short_answer";
  const isCorrect = autoGraded && normalizeAnswerText(submittedAnswer) === normalizeAnswerText(problem.correctAnswer);
  ...
  const submission = {
    ...
    problemType: type,
    answerText: type === "multiple_choice" ? selectedChoice : answerText,
    originalFileName,
    storedFileName,
    fileKind,
    mimeType,
    ...
    status: autoGraded ? (isCorrect ? "awarded" : "rejected") : "submitted",
    awardedPoints: autoGraded && isCorrect ? Number(problem.points ?? 10) || 10 : 0,
    autoGraded,
...
export async function scoreHallSubmission(submissionId, awardedPoints, status = "awarded", user = null) {
  ...
  const problem = problems.find((entry) => entry.id === submissions[index].problemId);

  if (user && !canGradeHallProblem(user, problem)) {
    throw new Error("이 문제를 채점할 권한이 없습니다.");
  }
  ...
  submissions[index] = {
    ...
    gradedByUserId: user?.id ?? submissions[index].gradedByUserId ?? null,
...
export async function getArticlePageBundle(slug, options = {}) {
  const { includeUnpublished = false, viewer = null } = options;
  const [{ articles, comments, hallProblems, issueMap }, hallSubmissions] = await Promise.all([
    loadCollections(),
    readCollection("hallSubmissions")
  ]);
  const article = articles.find((entry) => entry.slug === slug);
  ...
  const linkedProblems = hallProblems
    .filter((problem) => problem.articleSlug === slug || (article.linkedProblemIds ?? []).includes(problem.id))
    .map((problem) => {
      const canGrade = canGradeHallProblem(viewer, problem);
      const problemSubmissions = canGrade
        ? hallSubmissions
            .filter((submission) => submission.problemId === problem.id)
            .sort((left, right) => new Date(right.submittedAt) - new Date(left.submittedAt))
        : [];

      return sanitizeProblemForViewer(
        {
          ...problem,
