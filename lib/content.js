...
import { canCreateNotice, canPreviewArticle, canWriteArticles } from "@/lib/auth";
...
export async function listArticles(options = {}) {
  ...
  return sortByDate(
    articles.filter((article) => canAccessArticle(article, issueMap, viewer, includeUnpublished)),
    articleSortDate
...
export async function listBoardPosts() {
  ...
  return sortByDate(posts, (post) => post.createdAt);
}
...
export async function createHallSubmission(payload, user) {
  ...
  if (!problemId || !storedFileName || !originalFileName) {
    throw new Error("PDF 파일과 연결된 문제 정보가 필요합니다.");
  }
