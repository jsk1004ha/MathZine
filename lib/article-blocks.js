...
export const ARTICLE_HTML_MAX_LENGTH = 80_000;
export const ARTICLE_HTML_MIN_HEIGHT = 240;
export const ARTICLE_HTML_MAX_HEIGHT = 2400;
export const ARTICLE_HTML_DEFAULT_HEIGHT = 720;
...
export function createEmptyHtmlArticleDocument() {
  return {
    mode: "html",
    html: "",
    htmlHeight: ARTICLE_HTML_DEFAULT_HEIGHT,
    blocks: [],
    references: []
  };
}
...
export function clampArticleHtmlHeight(value) {
  const height = Number(value);

  if (!Number.isFinite(height)) {
    return ARTICLE_HTML_DEFAULT_HEIGHT;
  }

  return Math.min(ARTICLE_HTML_MAX_HEIGHT, Math.max(ARTICLE_HTML_MIN_HEIGHT, Math.round(height)));
}

function normalizeHtmlSource(value) {
  const source = String(value ?? "").replace(/\r\n?/g, "\n").trim();

  if (source.length > ARTICLE_HTML_MAX_LENGTH) {
    throw new Error(`HTML 코드는 ${ARTICLE_HTML_MAX_LENGTH.toLocaleString("ko-KR")}자 이하로 입력해 주세요.`);
  }
...
export function sanitizeArticleDocument(document) {
  if (document?.mode === "html") {
    return {
      mode: "html",
      html: normalizeHtmlSource(document.html),
      htmlHeight: clampArticleHtmlHeight(document.htmlHeight),
      blocks: [],
      references: []
    };
  }
...
export function normalizeArticleDocument(article) {
  if (article?.mode === "html") {
    return {
      mode: "html",
      html: String(article.html ?? ""),
      htmlHeight: clampArticleHtmlHeight(article.htmlHeight),
      blocks: [],
      references: []
    };
  }

  if (article?.document?.mode === "html") {
    return {
      mode: "html",
      html: String(article.document.html ?? ""),
      htmlHeight: clampArticleHtmlHeight(article.document.htmlHeight),
      blocks: [],
      references: []
    };
  }
  ...
  if (article?.blocks) {
    return {
      mode: article.mode === "legacy" ? "legacy" : "legacy",
  ...
  if (article?.document?.blocks) {
    return {
      mode: article.document.mode === "legacy" ? "legacy" : "legacy",
...
function htmlEntityToText(entity) {
  const namedEntities = {
    amp: "&",
    gt: ">",
    lt: "<",
  ...
  if (entity.startsWith("#x")) {
    return String.fromCodePoint(Number.parseInt(entity.slice(2), 16) || 0);
  }

  if (entity.startsWith("#")) {
    return String.fromCodePoint(Number.parseInt(entity.slice(1), 10) || 0);
  }
...
export function extractPlainTextFromHtml(html) {
  return String(html ?? "")
    ...
    .replace(/<[^>]+>/g, " ")
    .replace(/&([a-zA-Z]+|#\d+|#x[0-9a-fA-F]+);/g, (_match, entity) => htmlEntityToText(entity))
    .replace(/\s+/g, " ")
    .trim();
}
...
export function extractPlainTextFromDocument(document) {
  ...
  if (normalized.mode === "html") {
    return extractPlainTextFromHtml(normalized.html);
  }
...
export function getDocumentPullQuote(document) {
  ...
  if (normalized.mode === "html") {
    return extractPlainTextFromHtml(normalized.html).slice(0, 180);
  }
